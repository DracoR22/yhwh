...
impl Skin {
    ...
    pub fn compute_joints_matrices(&mut self, transform: cgmath::Matrix4<f32>, nodes: &[Node]) {
        self.joints
            .iter_mut()
...
impl Joint {
    fn new(inverse_bind_matrix: cgmath::Matrix4<f32>, node_id: usize) -> Self {
        Joint {
    ...
    fn compute_matrix(&mut self, transform: cgmath::Matrix4<f32>, nodes: &[Node]) {
...
fn map_inverse_bind_matrices(gltf_skin: &GltfSkin, data: &[Data]) -> Vec<cgmath::Matrix4<f32>> {
    let iter = gltf_skin
        .reader(|buffer| Some(&data[buffer.index()]))
        .read_inverse_bind_matrices()
        .expect("IBM reader not found for skin");
    iter.map(cgmath::Matrix4::from).collect::<Vec<_>>()
}
