...
use crate::{animation::animation::PlaybackMode, egui_renderer::ui_manager::EguiMaterial, game::game_data::GameData};
            ...
            for animated_game_object in game_data.scene.animated_game_objects.iter_mut() {
                ...
                if changed_x || changed_y || changed_z {
                    if self.scale_uniform {
                        ...
                        animated_game_object.transform.size = cgmath::Vector3::new(new_value, new_value, new_value);
                    } else {
                ...
                let selected_anim_index =  self.selected_anim_map.entry(animated_game_object.id).or_insert(0);
                let selected_anim = &animated_game_object.animations.as_ref().unwrap().animations()[*selected_anim_index];

                let mut value_changed = false;
                egui::ComboBox::from_label("")
                 .selected_text(selected_anim.get_name())
                 .show_ui(ui, |ui| {
                    for (i, anim) in animated_game_object.animations.as_ref().unwrap().animations().iter().enumerate() {
                        value_changed |= ui.selectable_value(selected_anim_index, i, anim.get_name()).changed();
                    }
                 });
                ...
                ui.add(egui::ProgressBar::new(play_back_state.progress())
                .text(format!(
                    "{:.2}s / {:.2}s",
                    play_back_state.time,
                    play_back_state.total_time
                )));

                let mut loop_anim = matches!(
                    play_back_state.playback_mode,
                    PlaybackMode::Loop
                );
