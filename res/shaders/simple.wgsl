...
fn fs_main(in: VertexOutput) -> @location(0) vec4<f32> {
    ...
    var normal_texture: vec3<f32> = textureSample(t_normal, s_normal, in.tex_coords).xyz * 2.0 - 1.0;

    let world_normal = normalize(mat3x3<f32>(in.tangent, in.bitangent, in.normal) * normal_texture);
    let ambient_strength = 0.1;
    let ambient_color = light.color * ambient_strength;

    let light_dir = normalize(light.position - in.world_position);
    let attenuation = smoothstep(10.0, 0.0, length(light.position - in.world_position));
    let diffuse_strength = max(dot(world_normal, light_dir), 0.0);
    let diffuse_color = light.color * attenuation * diffuse_strength;
